...
export class ApexFileEditor extends MonacoEditor{
    ...
    static CHANGES_PENDING_CLASS = 'fam-blank';
    ...
    evaluateChangeIcon(){
        ...
        if(this.initialValue != this.editor.getValue()){
            iconSpan.classList.add(ApexFileEditor.CHANGES_PENDING_CLASS);
        } else {
            iconSpan.classList.remove(ApexFileEditor.CHANGES_PENDING_CLASS);
        }
    }
